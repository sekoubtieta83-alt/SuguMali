...
export function PhoneLogin({ mode }: PhoneLoginProps) {
  ...
  const authRef = useRef(getAuth(getApp()));
  ...
  const initVerifier = () => {
    ...
    try {
      ...
      verifierRef.current = new RecaptchaVerifier(authRef.current, 'recaptcha-container', {
        size: 'invisible',
      });
    } catch (e) {
  ...
  const onSendOTP = async () => {
    ...
    setIsLoading(true);
    setLoadingMsg('Envoi du code…');
    setStep('loading');
    initVerifier();

    try {
      const confirmation = await signInWithPhoneNumber(authRef.current, formattedNumber, verifierRef.current!);
  ...
  const onVerifyOTP = async () => {
    ...
    try {
      ...
      setLoadingMsg('Redirection en cours…');
