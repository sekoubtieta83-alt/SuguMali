...
export function LoginForm() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    try {
      ...
      // Gestion des codes d'erreur précis pour le débogage
      switch (error.code) {
        ...
        case 'auth/operation-not-allowed':
          message = "La connexion par email n'est pas activée dans Firebase.";
          break;
  ...
  return (
    <Card className="w-full max-w-md shadow-2xl rounded-[2.5rem] border-none bg-card/80 backdrop-blur-sm overflow-hidden">
      ...
      <CardHeader className="space-y-2 text-center pt-10">
        <div className="flex justify-center items-center gap-3 mb-2 transition-transform hover:scale-105 duration-300">
          <Logo className="h-12 w-12 text-primary" />
          <CardTitle className="text-4xl font-black tracking-tighter text-foreground">Sugu<span className="text-primary">Mali</span></CardTitle>
        </div>
      ...
      <CardContent className="grid gap-6 px-8 pb-12">
        {authError && (
          <div className="p-4 rounded-2xl bg-destructive/10 text-destructive text-sm flex gap-3 items-center border border-destructive/20 animate-in fade-in slide-in-from-top-1">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-5">
                  ...
                  <FormControl>
                    <div className="relative">
                      ...
                      <Input 
                        ...
                        {...field} 
                        className="h-14 rounded-2xl bg-muted/50 border-none px-12 focus-visible:ring-primary/50 text-base" 
                      />
                    </div>
                  </FormControl>
            ...
            <Button 
              type="submit" 
              className="w-full bg-primary hover:bg-primary/90 text-white font-black h-14 rounded-2xl text-lg mt-2 shadow-xl shadow-primary/20 transition-all active:scale-[0.98]" 
              disabled={isLoading}
            >
