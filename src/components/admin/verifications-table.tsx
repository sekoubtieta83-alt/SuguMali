...
import { BadgeCheck, Loader2, Eye, Check, X, ShieldCheck, Wallet, Image as ImageIcon } from 'lucide-react';
                ...
                <TableRow key={u.id} className="hover:bg-muted/5 transition-colors">
                  ...
                  <TableCell>
                      <div className="flex gap-2">
                        {/* Dialogue pour la Pièce d'Identité */}
                        <Dialog>
                            <DialogTrigger asChild>
                                <Button variant="outline" size="sm" className="rounded-xl font-bold h-8 text-[10px]">
                                    <Eye className="h-3 w-3 mr-1" /> Voir l'ID
                                </Button>
                            </DialogTrigger>
                            <DialogContent className="max-w-2xl rounded-3xl overflow-hidden">
                                ...
                                <div className="bg-muted rounded-2xl overflow-hidden border flex items-center justify-center p-2">
                                    <img src={u.idDocumentUrl} alt="Pièce d'identité" className="w-full h-auto object-contain max-h-[70vh] rounded-xl" />
                                </div>
                            </DialogContent>
                        </Dialog>

                        {/* Dialogue pour le Reçu de Paiement */}
                        {u.paymentScreenshotUrl && (
                          <Dialog>
                              <DialogTrigger asChild>
                                  <Button variant="secondary" size="sm" className="rounded-xl font-bold h-8 text-[10px] bg-accent/10 text-accent hover:bg-accent/20">
                                      <ImageIcon className="h-3 w-3 mr-1" /> Voir Reçu
                                  </Button>
                              </DialogTrigger>
                              <DialogContent className="max-w-2xl rounded-3xl overflow-hidden">
                                  <DialogHeader>
                                    <DialogTitle>Preuve de paiement - {u.displayName}</DialogTitle>
                                  </DialogHeader>
                                  <div className="bg-muted rounded-2xl overflow-hidden border flex items-center justify-center p-2">
                                      <img src={u.paymentScreenshotUrl} alt="Preuve de paiement" className="w-full h-auto object-contain max-h-[70vh] rounded-xl" />
                                  </div>
                              </DialogContent>
                          </Dialog>
                        )}
                  ...
                  <TableCell className="text-right pr-6">
                    <div className="flex justify-end gap-2">
                      <Button size="sm" onClick={() => handleAction(u.id, 'verified')} className="bg-accent text-white hover:bg-accent/90 rounded-xl font-bold h-8 px-4">
                          <Check className="h-4 w-4 mr-1" /> Valider
                      </Button>
                      <Button variant="ghost" size="sm" onClick={() => handleAction(u.id, 'rejected')} className="text-destructive hover:bg-destructive/10 rounded-xl font-bold h-8 px-4">
