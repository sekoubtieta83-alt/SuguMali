/**
 * @fileOverview Flux backend d'analyse d'image par Mami.
 * Reçoit une image en base64 et retourne un titre, une description et une catégorie suggérés.
 */
...
- Réponds UNIQUEMENT avec le JSON, rien d'autre`;
  ...
  if (!res.ok) {
    ...
    console.error('Gemini Vision Backend Error:', errText);
  ...
  // Nettoyer et parser le JSON (Gemini peut parfois ajouter du markdown)
  const clean = raw.replace(/```json|```/g, '').trim();
  try {
    return JSON.parse(clean);
  } catch (e) {
    console.error('Erreur parsing JSON Mami Vision:', clean);
    return {
      titre: '',
      description: '',
      categorie: 'Autre',
